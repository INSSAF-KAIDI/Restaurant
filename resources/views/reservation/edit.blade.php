...
<body>
    ...
    <div class="container ">
        ...
        <form action="{{ route('reservation.update',$reservation->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="card border-0 shadow-lg">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="NomClient" id="NomClient" class="form-select
                         @error('NomClient') is-invalid @enderror">
                            ...
                            <option value="{{ $client->id }}" {{ $reservation->client_id== $client->id ? "selected":'' }}>{{ $client->NomClient }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="NumTable" id="NumTable" class="form-select
                         @error('NumTable') is-invalid @enderror">
                            ...
                            <option value="{{ $table->id }}" {{ $reservation->table_id== $table->id ? "selected":'' }}>{{ $table->NumTable }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="DateDemande" id="DateDemande" placeholder="Entrer DateDemande" class="form-control @error('DateDemande') is-invalid @enderror" value="{{ old('DateDemande',$reservation->DateDemande) }}">
                        @error('DateDemande')
                            <p class="invalid-feedback">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <button class="btn btn-primary my-3">Update Reservation</button>
