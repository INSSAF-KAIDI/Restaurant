...
@section('title', 'Create Category Alimentaire')
...
<body>
    ...
    <div class="container ">
        <div class="d-flex justify-content-between py-3">
            <div class="h4">Category Alimentaires</div>
            <div>
                <a href="{{ route('categoryalimentaire.index') }}" class="btn btn-primary">Back</a>
            </div>
        </div>

        <form action="{{ route('categoryalimentaire.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="card border-0 shadow-lg">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="NomCategory" id="NomCategory" class="form-select
                         @error('NomCategory') is-invalid @enderror">
                            ...
                            <option value="{{ $Category->id }}" {{ old('NomCategory') == $Category->id ? "selected":'' }}>{{ $Category->NomCategory }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="NomArticle" id="NomArticle" placeholder="Entrer NomArticle" class="form-control @error('NomArticle') is-invalid @enderror" value="{{ old('NomArticle') }}">
                        @error('NomArticle')
                            <p class="invalid-feedback">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <button class="btn btn-primary mt-3">Save Category Alimentaire</button>
